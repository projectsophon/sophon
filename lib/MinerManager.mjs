...
export class MinerManager extends EventEmitter {
  ...
  constructor(minedChunksStore, miningPattern, worldRadius, planetRarity) {
    super();
  ...
  async nextValidExploreTarget(chunkLocation, jobId) {
    ...
    // hack to only mine deep space perlin
    // todo add ui to enable somehow
    // TODO WARNING HARDCODING ContractConstants.PERLIN_THRESHOLD_2
    let perlin = perlin(candidateChunk.center, false);
    if (this.isValidExploreTarget(candidateChunk) && perlin >= 17) {
