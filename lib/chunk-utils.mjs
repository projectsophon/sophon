...
export const getChunkKey = (chunkLoc) => {
  const ret =
    `${getBucket(chunkLoc)},` +
    `${chunkLoc.sideLength},` +
...
export const toExploredChunk = (chunk) => {
  const planetLocs = [];
...
export const getSiblingLocations = (chunkLoc) => {
...
export const getChunkOfSideLength = (coords, sideLength) => {
...
export const addToChunkMap = (map, chunk, includePlanets = true, onAdd, onRemove, maxChunkSize) => {
  ...
  while (!maxChunkSize || sideLength < maxChunkSize) {
    ...
    sideLength *= 2;
    let planetLocations = chunkToAdd.planetLocations;
